...
def extract_largest_float(s):
    ...
    if fl:
        ...
        return bytes(max([float(f) for f in fl]))
    return ""
...
def item_dict_to_string(dic, header=None):
    ...
    field_list.sort()
    output_list = [header] + ["\t%s: %s" % (f[0], f[1]) for f in field_list]
    return "\n".join(output_list)
...
class NrcItem(Item):
    ...
    def __repr__(self):
        return "%s(%s)" % (
            ...
            ", ".join(["%s=%s" % (k, self.get(k)) for k in self.keyFields()]),
        )
...
class NrcScrapedReport(Item):
    ...
    def __repr__(self):
        return "NrcScrapedReport(reportnum=%s, incident_datetime=%s)" % (
            self["reportnum"],
            self["incident_datetime"],
        )
...
class NrcParsedReport(Item):
    ...
    latitude = SingleField(input_processor=MapCompose(convert_lat))
    longitude = SingleField(input_processor=MapCompose(convert_lng))
    zip = SingleField()
...
class NrcScrapedMaterial(Item):
    ...
    chris_code = SingleField(
        xpath='td[@headers="Chris Code"]/text()', input_processor=MapCompose(str.strip)
    )
    name = SingleField(
...
class PA_Violation(NrcItem):
    ...
    # Cannot truncate here...
    # InspectionResult = (ContentField () [:252] + '..') if len(ContentField ()) > 254 else ContentField ()
...
class FracFocusParse(NrcItem):
    ...
    seqid = SingleField()
    api = KeyField(input_processor=MapCompose(normalize_api))
    fracture_date = KeyField(input_processor=MapCompose(convert_fuzzy_date))
    state = SingleField()
    county = SingleField()
    operator = SingleField()
    well_name = SingleField()
    ...
    longitude = SingleField(input_processor=MapCompose(convert_lng))
    datum = ContentField()
    production_type = SingleField(input_processor=MapCompose(title_case))
    true_vertical_depth = SingleField(input_processor=MapCompose(strip_non_digits))
    total_water_volume = SingleField(input_processor=MapCompose(strip_non_digits))
