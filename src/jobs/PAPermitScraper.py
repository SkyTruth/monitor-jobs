        ...
        try:
            ...
            for child in element:
                for p in child.getiterator():
                    ...
                    if p.tag == 'P_START_DATE': P_START_DATE = p.text
                    if p.tag == 'P_END_DATE': P_END_DATE = p.text
                    if p.tag == 'P_COUNTY': P_COUNTY = p.text
                    if p.tag == 'P_MUNICIPALITY': P_MUNICIPALITY = p.text
                    if p.tag == 'P_REGION': P_REGION = p.text
                    if p.tag == 'P_SEARCH_NAME': P_SEARCH_NAME = p.text
                    if p.tag == 'P_UNCONVENTIONAL': P_UNCONVENTIONAL = p.text
                    if p.tag == 'P_WELL_TYPE': P_WELL_TYPE = p.text
                    if p.tag == 'REGION': REGION = p.text
                    if p.tag == 'COUNTY': COUNTY = p.text
                    if p.tag == 'MUNICIPALITY': MUNICIPALITY = p.text
                    if p.tag == 'PERMIT_ISSUED_DATE': PERMIT_ISSUED_DATE = p.text.replace('T', ' ')
                    if p.tag == 'OPERATOR': OPERATOR = p.text
                    if p.tag == 'APPLICATION_TYPE': APPLICATION_TYPE = p.text
                    if p.tag == 'AUTH_TYPE_DESCRIPTION': AUTH_TYPE_DESCRIPTION = p.text
                    if p.tag == 'WELL_API': WELL_API = p.text
                    if p.tag == 'UNCONVENTIONAL': UNCONVENTIONAL = p.text
                    if p.tag == 'CONFIGURATION': CONFIGURATION = p.text
                    if p.tag == 'WELL_TYPE': WELL_TYPE = p.text
                    if p.tag == 'FARM_NAME': FARM_NAME = p.text
                    if p.tag == 'SPUD_DATE': SPUD_DATE = p.text
                    if p.tag == 'LATITUDE_DEGREES': LATITUDE_DEGREES = p.text
                    if p.tag == 'LONGITUDE_DEGREES': LONGITUDE_DEGREES = p.text
                    if p.tag == 'LATITUDE_DECIMAL': LATITUDE_DECIMAL = p.text
                    if p.tag == 'LONGITUDE_DECIMAL': LONGITUDE_DECIMAL = p.text
                    if p.tag == 'OGO_NUM': OGO_NUM = p.text
                    if p.tag == 'OPERATOR_ADDRESS': OPERATOR_ADDRESS = p.text
                    if p.tag == 'CITY': CITY = p.text
                    if p.tag == 'STATE': STATE = p.text
                    if p.tag == 'ZIP_CODE': ZIP_CODE = p.text
                    if p.tag == 'AUTHORIZATION_ID': AUTHORIZATION_ID = p.text
                    if p.tag == 'CLIENT_ID': CLIENT_ID = p.text
                    if p.tag == 'PRMRY_FAC_ID': PRMRY_FAC_ID = p.text
                    if p.tag == 'MARCELLUS_SHALE_WELL': MARCELLUS_SHALE_WELL = p.text
            ...
            summary = WELL_TYPE + \
                      ...
                      " for site " + \
                      FARM_NAME + \
                      " in " + MUNICIPALITY + " township" + \
                      ", " + COUNTY + " county"

            content = "<b>Report Details</b>" + \
                      "<table>" + \
                      "<tr><th>Well Type:</th><td>" + WELL_TYPE + "</td></tr>" + \
                      "<tr><th>Permit Issued:</th><td>" + PERMIT_ISSUED_DATE + "</td></tr>" + \
                      ...
                      "<tr><th>Permit Type:</th><td>" + APPLICATION_TYPE + "</td></tr>" + \
                      "<tr><th>Description:</th><td>" + AUTH_TYPE_DESCRIPTION + "</td></tr>" + \
                      "<tr><th>Unconventional:</th><td>" + UNCONVENTIONAL + "</td></tr>" + \
                      "<tr><th>Horizontal:</th><td>" + horiz + "</td></tr>" + \
                      "<tr><th>Total Depth:</th><td>" + "</td></tr>" + \
                      "<tr><th>Well API Number:</th><td>" + WELL_API + "</td></tr>" + \
                      "<tr><th>OGO Number:</th><td>" + OGO_NUM + "</td></tr>" + \
                      "<tr><th>Facility ID:</th><td>" + PRMRY_FAC_ID + "</td></tr>" + \
                      "</table>"
