...
if __name__ == '__main__':
            ...
            try:
                ...
                try:
                    if emails_sent < 2000:
                        ...
                        server.close()
                        print ('successfully sent the mail to ' + email + ' updating ' + aoiid, aoidescr )
                        ...
                        if is_id > 0:
                            db.upd_issuesubscription_last_email_sent(str(is_id), str(last_published))
                except Exception as exception:
                    exc_info = sys.exc_info()
                    exc_type, exc_obj, exc_tb = exc_info
                    fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                    error = ("failed to send mail to ", msg['To'], str(exception),
                ...
                exc_type, exc_obj, exc_tb = exc_info
                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                error = ("Exception while creating email for " + email + ": ", str(exception2),
                         str(exc_type), str(fname), str(exc_tb.tb_lineno),
                         " see logfile for more info")
                print('error:', error)
                print('title:', title)
    ...
    utils.send_email('alerts emailed (' + str(emails_sent) + ')', str(emails_sent) + ' emails sent with ' +
                     str(total_alerts_included) + ' alerts' + ' and ' + str(dates_updated) + ' updates')
